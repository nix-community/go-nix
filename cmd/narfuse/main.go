...
// This was adapted from the main program driver for
// github.com/hanwen/go-fuse/zipfs, a filesystem for mounting read-only
// archives.
package main
...
import (
	...
	"github.com/nix-community/go-nix/pkg/nar/narfuse"
)
...
func main() {
	...
	flag.Parse()
	if flag.NArg() < 2 {
		fmt.Fprintf(os.Stderr, "usage: %s MOUNTPOINT NAR-FILE\n", os.Args[0])
		os.Exit(2)
	}
	...
	root, err := narfuse.New(flag.Arg(1))
	if err != nil {
		fmt.Fprintf(os.Stderr, "narfuse.New failed: %v\n", err)
		os.Exit(1)
	}
